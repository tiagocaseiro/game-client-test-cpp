...
public:
    ...
    {
        if(auto it = std::find_if(mTextures.begin(), mTextures.end(),
                                  [&path](const auto& desc) {
...
TextureManager::TextureManager(SdlWindow& window) : mImpl(new TextureManagerImpl(window.Renderer()))
{
}
