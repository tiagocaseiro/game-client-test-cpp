...
bool RaycastCircle(const glm::vec2& origin, const glm::vec2& direction, const glm::vec2& center, const float radius, glm::vec2& point, glm::vec2& normal) {
	assert(radius > 0);
...
bool RaycastRectWithRadius(const glm::vec2& origin, const glm::vec2& direction, const BoxCollider& box, const float radius, glm::vec2& point, glm::vec2& normal) {
...
int CollisionWorld::AddBoxCollider(const glm::vec2& topLeft, const glm::vec2& size, unsigned int layer, unsigned int mask) {
...
void CollisionWorld::ProcessCollisions(float dt) {
	...
	for (const auto& rigidbody: mRigidBodies) {
		...
		for (const auto& collider : rigidbody->mColliders) {
			...
			if(colliderInfo.type == ColliderType::Circle) {
				...
				while (distanceToMove > 0 && CircleCast((*sphereColliderIter).second.mPosition,
														direction,
														(*sphereColliderIter).second.mRadius,
														distanceToMove,
														result,
														colliderInfo.mMask)) {
	...
	auto c2b = [](const CircleCollider& a, const BoxCollider& b) -> bool {
        ...
        if (circleDistance.x > (b.mSize.x*0.5f + a.mRadius) ||
            circleDistance.y > (b.mSize.y*0.5f + a.mRadius)) {
            // Circle is far enough from the box
            return false;
        }
        
        if (circleDistance.x <= (b.mSize.x*0.5f) ||
            circleDistance.y <= (b.mSize.y*0.5f)) {
            // Circle is colliding with the box
            return true;
        }

        // Check corners collision
        float cornerDistance_sq = pow(circleDistance.x - b.mSize.x*0.5f, 2.0f) + pow(circleDistance.y - b.mSize.y*0.5f, 2.0f);
