...
void SdlSurface::Blit(SdlWindow& window, float x, float y, float rotation, float scale)
{
    ...
    SDL_RenderCopyEx(&window.Renderer(), &*mTexture, &srcRect, &dstRect, static_cast<double>(rotation), &center,
...
void SdlSurface::BlitSub(SdlWindow& window, const Rect& source, const Rect& target)
{
    ...
    SDL_RenderCopyEx(&window.Renderer(), &*mTexture, &srcRect, &dstRect, 0.0, &center, SDL_RendererFlip::SDL_FLIP_NONE);
}
