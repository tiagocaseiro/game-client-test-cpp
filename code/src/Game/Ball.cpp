...
Ball::Ball(King::Engine& engine)
	: mEngine(engine)
	, mBallTxId(mEngine.LoadTexture("Ball-yellow.png"))
{
...
void Ball::Render() const {
	auto collider = mEngine.GetCollisionWorld().GetCircleCollider(mBallColliderId);
	mEngine.Render(mBallTxId,
				   collider->mPosition.x - kRadius,
				   collider->mPosition.y - kRadius,
				   0,
				   2);
}
...
void Ball::OnCollision(int l, int r) {
    ...
    if(hitBall && !mIsLevelClear) {
        ...
        if(bounceMagnitude < kBounceLimit) {
            float x = sqrt(1-std::pow(kBounceLimit, 2)); // Get x from a vector of lenght 1 and kBounceLimit as dot product with the up vector
